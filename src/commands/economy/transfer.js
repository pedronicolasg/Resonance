  ...
  run: async (client, interaction, args) => {
    ...
    try {
      ...
      const embed = new Discord.EmbedBuilder()
        ...
        .setDescription(
          `Você enviou \`${amount} ${ecoCfg.coinname}s\` para <@${recipientId}>.\nSeu saldo atual: \`${sender.coins} ${ecoCfg.coinname}s\``
        );

      interaction.reply({ embeds: [embed] });
      logger_economy.info(
        `${interaction.user.id} transferiu ${ecoCfg.coinsymb}:${amount} para ${recipientId} no servidor ${interaction.guild.id}`
      );
    } catch (e) {
      console.log(
        themes.error("Erro ") +
          `ao enviar ${ecoCfg.coinsymb}:${amount} de ${senderId} para ${recipientId} devido à:\n ${e}`
      );
      logger.error(
        `Erro ao enviar ${ecoCfg.coinsymb}:${amount} de ${senderId} para ${recipientId} devido à: ${e}`
      );
