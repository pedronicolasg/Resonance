    ...
    try {
      ...
      console.log(
        themes.success("Sucesso ") +
          `ao mudar status do bot para: ${status}-${desc}`
      );
      logger.info(`Status do bot mudou para: ${status}-${desc}`);
    } catch (e) {
      console.log(
        themes.error("Erro ") + `ao mudar status do bot devido à:\n ${e}`
      );
      logger.error(`Erro ao mudar status do bot devido à: ${e}`);
    }
  },
};
