  ...
  try {
    ...
    if (serverSettings && serverSettings.welcomechannelId) {
      ...
      if (welcomeChannel) {
        ...
        console.log(
          themes.error("Erro: ") +
            `Canal de boas-vindas com ID ${exitChannelId} não encontrado no servidor ${member.guild.name} (${member.guild.id})`
        );
        logger.error(
          `Erro: Canal de boas-vindas com ID ${exitChannelId} não encontrado no servidor ${member.guild.name} (${member.guild.id})`
        );
      }
    } else {
      console.log(
    ...
    console.log(
      themes.error("Erro ") + `ao buscar as configurações no MongoDB:\n ${e}`
    );
