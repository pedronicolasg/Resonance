  ...
  try {
    ...
    if (serverSettings && serverSettings.exitchannelId) {
      ...
      if (exitChannel) {
        ...
        console.log(
          themes.error("Erro: ") +
            `Canal de saída com ID ${exitChannelId} não encontrado no servidor ${member.guild.name} (${member.guild.id})`
        );
        logger.error(
          `Erro: Canal de saída com ID ${exitChannelId} não encontrado no servidor ${member.guild.name} (${member.guild.id})`
        );
      }
    } else {
      console.log(
